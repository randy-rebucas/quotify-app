                    ...
                    {refinementData.map((refinement: refinementType, index: any) => (
                        <div data-value={refinement.name} data-col={index + 1} key={refinement.id}
                            ...
                            <div className="p-30 estimation">
                                <input type="radio" name="refinement" value={refinement.slug} id={`refinement-${index + 1}`} onChange={e => handleRadioChange(index, e)} checked={stimates[tabiIndex].refinement.furniture == refinement.slug}/>
