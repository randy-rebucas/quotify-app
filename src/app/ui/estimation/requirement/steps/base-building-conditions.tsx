...
type buildingConditionFormProps = RequirementData & {
    stimates: any,
    tabiIndex: number,
...
export default function BaseBuildingConditions({
    ...
    useMemo(async () => {
        ...
        if (requirementId === undefined) {
            const filter = 'base building conditions';
    ...
    const handleRadioChange = (index: number, event: ChangeEvent<HTMLInputElement>) => {
        let data = [...stimates];
        data[tabiIndex].requirement.buildingCondition = event.target.value;
            ...
            {requirementLevels.map((requirementLevel: any, index: number) => (
                <div data-category={`03.1.${index + 1}`} key={requirementLevel._id.toString()} data-value={requirementLevel.level} data-col={index + 1}
                    ...
                    <div className="p-30 estimation">
                        <input type="radio" name="finish" value={requirementLevel.level} id={`finish-${index + 1}`} onChange={e => handleRadioChange(index, e)} checked={stimates[tabiIndex].requirement.buildingCondition == requirementLevel.level} />
