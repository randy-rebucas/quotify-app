...
export default function RefinementLevelOption({ refinement, hasRefinement, onChange }: { refinement: string; hasRefinement: string; onChange: ChangeEventHandler<HTMLInputElement> }) {
    
    const getRefinementIdByName = useRefinementStore(state => state.getRefinementIdByName);
    const refinementId = useRefinementStore(state => state.refinementId);

    const getRefinementLevelByRefinement = useRefinementLevelStore(state => state.getRefinementLevelByRefinement);
    const refinementLevels = useRefinementLevelStore(state => state.refinementLevels);
    ...
    }, [refinement, refinementId, getRefinementLevelByRefinement, getRefinementIdByName])
                ...
                {refinementLevels.map((refinementLevel: any, index: number) => (
                    <div data-value={refinementLevel.level} data-col={index + 1} key={refinementLevel._id.toString()}
                        ...
                        <div className="p-30 estimation estimation-yellow">
                            <input type="radio" name="refinement" value={refinementLevel._id.toString()} id={`refinement-${index + 1}`} onChange={onChange} checked={hasRefinement == refinementLevel._id.toString()} />
