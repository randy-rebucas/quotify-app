...
export default function FinishLevelOption({ requirement, hasRequirement, onChange }: { requirement: string; hasRequirement: string; onChange: ChangeEventHandler<HTMLInputElement> }) {
    ...
    useEffect(() => {
        getRequirementByName(requirement);

        if (requirementId) {
            getRequirementLevelByRequirement(requirementId)
        }

    }, [requirement, requirementId, getRequirementLevelByRequirement, getRequirementByName])

    return (
        <>
            {requirementLevels.length && requirementLevels.map((requirementLevel: any, index: any) => {
                return (
                    <>
                        <div data-category={`03.1.${index + 1}`} key={requirementLevel._id.toString()} data-value={requirementLevel.level} data-col={index + 1}
                            ...
                            <div className="p-30 estimation estimation-green">
                                <input type="radio" name="finish" value={requirementLevel._id.toString()} data-label={requirementLevel.level} id={`finish-${index + 1}`} onChange={onChange} checked={hasRequirement == requirementLevel._id.toString()} />
